// The handoff to quill-cloud-proxy/tools/deploy-azure-aci.sh. Everything the
// measured deploy needs to know about a region is derived here rather than
// retyped, so a new region cannot be deployed against a stale MAA endpoint or
// the wrong identity.

output "deploy_env" {
  description = <<-EOT
    Per-region environment for quill-cloud-proxy/tools/deploy-azure-aci.sh.
    Render one region's block and export it, e.g.:
