...
`quill-cloud-proxy/tools/deploy-azure-aci.sh` warns about in its header: a
confidential group must be deleted before it can be recreated, so a create that
fails for an ordinary reason (ACR pull, capacity, quota) leaves no group *and*
no way back.
...
The shared `TR-TEE-DUBAI` resource group is never managed here either. It holds
the vault, registry, and storage account used by every region, so putting it
under a per-region lifecycle would put the whole Azure enclave fleet at risk.
...
Azure Blob, using the same Azure-local storage account and container as
`envs/azure-prod` with a separate state key. This keeps Azure provisioning
independent of AWS and GCP.

The module used local state in `quill-cloud-proxy` by design. Existing operators
must migrate that state exactly once with `terraform init -migrate-state`
before planning or applying here; follow [`MIGRATION.md`](MIGRATION.md).
...
Export those and run the measured deploy from the `quill-cloud-proxy` repo root:
