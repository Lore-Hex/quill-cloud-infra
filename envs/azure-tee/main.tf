...
// failure quill-cloud-proxy/tools/deploy-azure-aci.sh's header warns about: a
// confidential group must be deleted before it can be recreated, so a create
// that fails for an ordinary reason (ACR pull, capacity, quota) would leave no
// group AND no way
...
// STATE
// ---------------------------------------------------------------------------
// The original module used local state by design: putting Azure's bring-up
// state in AWS or GCP would make one cloud a prerequisite for provisioning
// another -- the exact hub dependency the multi-cloud split exists to avoid.
// This Azure-local backend uses the same storage account and container as
// envs/azure-prod, with a separate key for this root module.
//
// MIGRATION (one-time): copy the old
// quill-cloud-proxy/tools/azure-enclave/terraform.tfstate into this directory,
// then run `terraform init -migrate-state` here. See MIGRATION.md for the full
// operator sequence. Do not delete the old directory until `terraform plan`
// reports "No changes" against the migrated state.
//
// Moving state does not change the ownership boundary: this module NEVER
// manages enclave container groups, the tr-bootstrap-wrap SKR key, or the
// shared TR-TEE-DUBAI resource group.
//
...
terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "envs/azure-tee/terraform.tfstate"
...
resource "azurerm_attestation_provider" "maa" {
  ...
  lifecycle {
    ...
    // quill-cloud-proxy/tools/deploy-azure-aci.sh. These are MAA's own defaults
    // for evaluating a report; we neither set nor version them, so tracking
    // them here would report drift we have no opinion about.
    ignore_changes = [
