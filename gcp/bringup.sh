...
if [ -z "${IMAGE_TAG:-}" ]; then
  if git -C "$PROXY_DIR" rev-parse --short HEAD >/dev/null 2>&1; then
    IMAGE_TAG="gcp-release-$(git -C "$PROXY_DIR" rev-parse --short HEAD)"
  else
    IMAGE_TAG="latest"
  fi
fi
...
# Project-level grants the workload SA needs:
#   - artifactregistry.reader: pull the workload image
#   - logging.logWriter:       write structured logs to Cloud Logging
#                              (without this, the launcher boots and pulls
#                              the image but spams "PermissionDenied" on
#                              every log flush — cosmetic but noisy)
log "granting project-level roles to workload SA..."
# - artifactregistry.reader:               pull the workload image
# - logging.logWriter:                     write to Cloud Logging (cosmetic)
# - confidentialcomputing.workloadUser:    let the CSP launcher's REST
#                                          verifier client list confidential-
#                                          computing locations (without it
#                                          the launcher exits exit_code=4
#                                          with "Permission ... denied").
for role in "roles/artifactregistry.reader" "roles/logging.logWriter" "roles/confidentialcomputing.workloadUser"; do
...
if [ -x "$PROXY_DIR/tools/release-gcp.sh" ]; then
  log "building formal GCP release via quill-cloud-proxy/tools/release-gcp.sh"
  (
    cd "$PROXY_DIR"
...
if ! gc compute instances describe "$VM_NAME" --zone "$ZONE" >/dev/null 2>&1; then
  ...
  gc compute instances create "$VM_NAME" \
    ...
    # tee.launch_policy.allow_env_override LABEL, and we deliberately
    # don't whitelist it because flipping TLS off-by-runtime would
    # silently break the trust chain.
else
...
if ! gc compute firewall-rules describe quill-allow-lb-health >/dev/null 2>&1; then
  ...
  gc compute firewall-rules update quill-allow-lb-health --rules="tcp:443" >/dev/null
fi
if ! gc compute firewall-rules describe quill-allow-public-tls >/dev/null 2>&1; then
  ...
  gc compute firewall-rules update quill-allow-public-tls --rules="tcp:443" >/dev/null
fi

# ---- 9. Public IP + DNS ----------------------------------------------
# For V1 we just publish the VM's external IP; the user adds an A record
# in Cloudflare DNS for api.quillrouter.com. (Skipping the L4 GLB
# for the first cut — direct VM IP is simpler and TCP passthrough on
# :443 is what the workload expects anyway.)
...
Next steps:
  1. Add A record in Cloudflare DNS:
       $API_HOST  →  $EXTERNAL_IP   (DNS-only, NOT proxied)
  ...
  3. Smoke test:
       curl -sS https://$API_HOST/attestation -o /tmp/att.jwt
       BEARER=\$(cat $SECRET_DEVICES_BEARER_FILE | tr -d '[:space:]')
       curl -sS -N "https://$API_HOST/v1/chat/completions" \\
         -H "Authorization: Bearer \$BEARER" \\
         -H "content-type: application/json" \\
         -d '{"model":"claude-opus-4-7","stream":true,
              "messages":[{"role":"user","content":"reply PONG"}]}'

  4. Confirm the published trust files match this deploy:
       $PROXY_DIR/trust-page/image-reference-gcp.txt
       $PROXY_DIR/trust-page/image-digest-gcp.txt
