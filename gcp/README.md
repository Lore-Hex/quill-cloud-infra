...
6. Builds + pushes a formal `enclave-openrouter:gcp-release-<commit>`
   image from `quill-cloud-proxy/enclave-go/Dockerfile.enclave.gcp.openrouter`
   through `quill-cloud-proxy/tools/release-gcp.sh`.
...
8. Opens firewall ingress on `tcp:443`.
...
```
api.quillrouter.com  A  <IP from bringup.sh output>
```
...
| Hop | Property |
|---|---|
| Client → GCE network | TLS, hostname pinned to api.quillrouter.com |
...
`bringup.sh` also updates the proxy repo's GCP trust files:

- `trust-page/image-reference-gcp.txt`
- `trust-page/image-digest-gcp.txt`
- `trust-page/gcp-release.json`

Those files are committed in `quill-cloud-proxy`; its deploy workflow signs
them with keyless cosign and publishes them to `trust.quill.lorehex.co`.
The device uses `image-digest-gcp.txt` as the expected value when verifying
the live Confidential Space JWT before prompt traffic.
